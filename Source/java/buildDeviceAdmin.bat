...
"C:\Program Files (x86)\Java\jdk1.8.0_181\bin\javac" -source 1.7 -target 1.7 %VERBOSE_FLAG% -Xlint:deprecation -cp %ANDROID_PLATFORM%\android.jar -d output\classes src\android\app\admin\DeviceAdminReceiver.java
...
"C:\Program Files (x86)\Java\jdk1.8.0_181\bin\jar" c%VERBOSE_FLAG%f output\jar\DeviceAdminReceiver_classes.jar -C output\classes com
 
echo.
echo Now we have the end result, which is output\jar\DeviceAdminReceiver_classes.jar
